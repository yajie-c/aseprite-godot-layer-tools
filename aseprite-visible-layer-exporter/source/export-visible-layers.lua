...
local function choose_metadata_path(sprite)
  ...
  local dlg = Dialog{ title="Export Visible Layers (Trimmed)" }
  dlg:label{
    text="Exports every visible pixel layer in the current frame as a cropped PNG.\n"
      .. "Choose where to save layers.json; PNG files are saved beside it."
  }
  dlg:file{
    id="metadata_path",
    label="Export location",
    title="Save layers.json",
    save=true,
...
local function export_visible_layers()
  ...
  if not sprite then
    app.alert("Open a sprite before exporting layers.")
    return
  end
  ...
  if #layers_to_export == 0 then
    app.alert("There are no visible pixel layers to export.")
    return
  end
  ...
  if not ok then
    app.alert{ title="Export failed", text=tostring(failure) }
    return
  end

  local message = string.format("Exported %d layer(s) to:\n%s",
    #exported_layers, output_directory)
  if empty_count > 0 then
    message = message .. string.format("\nSkipped %d empty layer(s).", empty_count)
  end
  app.alert{ title="Export complete", text=message }
end

function init(plugin)
  plugin:newCommand{
    id="TrimmedVisibleLayerExporter",
    title="Export Visible Layers (Trimmed)",
    group="file_scripts",
    onenabled=function()
